...
import { proofreadEnglish, repeatedTemporalClause } from "./proofread.mjs";
  ...
  for (const file of readdirSync(units).filter((name) => /^(18t-g|35r-g|51q-g|57g-g).*\.yaml$/.test(name))) {
    ...
    if (semanticReview) {
      ...
      for (let attempt = 0; attempt < 12 && !corrected; attempt++) corrected = buildUnit({
        ...pair, index: 1, seed: "review:v8:" + unit.id + ":" + attempt, seen,
    ...
    if (JSON.stringify(unit) === JSON.stringify(reviewed)) continue;
